...
from topic.models import Comments
...
class ThankReplyView(View):
    @method_decorator(login_auth)
    def dispatch(self, request, *args, **kwargs):
        return super(ThankReplyView, self).dispatch(request, *args, **kwargs)
    ...
    def post(self, request):
        ...
        if author.is_superuser:
            ret['msg'] = '超级管理员不需要感谢'
            return HttpResponse(json.dumps(ret))
        author_detail = author.user_details.first()
        author_detail.balance += 10
        author_detail.save()

        user = get_object_or_404(User, id=user_id)
        user_detail = user.user_details.first()
        user_detail.balance -= 10
        user_detail.save()
        ...
        ret['changed'] = True
        request.session['user_info']['balance'] = user_detail.balance
        return HttpResponse(json.dumps(ret))
    ...
    def post(self, request):
        ...
        if obj.is_valid():
            ...
            if user_obj.check_password(password):
                ...
                status = send_email_code(to=new_email, code=random_code)
                if status:
    ...
    def post(self, request, ):
        ...
        if send_type and send_to:
            ...
            if not code_obj or int((current_time - code_obj.add_time).total_seconds()) > 60:
                ...
                from utils.send_notify_mail import send_email_code
                status = send_email_code(to=send_to, code=random_code)
