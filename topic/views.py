...
class IndexView(View):
    def get(self, request):
        ...
        if current_tab == 'hot':
            ...
            topic_obj = Topic.objects.select_related('author', 'category').all().order_by('-is_sticky', '-comment_num')[
                        0:30]
        ...
        if current_tab == 'tech':
            ...
            topic_obj = Topic.objects.select_related('author', 'category').all().order_by('-is_sticky', '-add_time')[
                        0:30]
        else:
            topic_obj = Topic.objects.select_related('author', 'category').filter(
                category__parent_category__code=current_tab).order_by('-is_sticky', '-add_time')[0:30]
        return render(request, 'topic/index.html', locals())
...
class TopicView(View):
    def get(self, request, topic_sn):
        try:
            ...
            comments_obj = Comments.objects.select_related('author').filter(topic=topic_obj).order_by('-thank_num')
            now = datetime.now()
            if request.session.get('user_info'):
